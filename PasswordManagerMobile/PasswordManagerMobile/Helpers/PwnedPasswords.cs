...
namespace PasswordManagerApp.Handlers
{
    ...
    public  class PwnedPasswords
    {
        ...
        /// <returns></returns>
        public  static int IsPasswordPwnedAsync(string password, CancellationToken cancellationToken, HttpClient httpClient = null)
        {
            ...
            try
            {
                ...
                var response = client.GetAsync($"https://api.pwnedpasswords.com/range/{hashFirstFive}").Result;
                var responseString = response.Content.ReadAsStringAsync().Result;
                return ReduceResult(hashString, responseString);
            }
            catch (Exception)
            {
                return -1;
        ...
        {
            string[] hashes = listOfHashes.Split(Environment.NewLine.ToCharArray());
